<template>
  <div class="v_slider">
    <div class="v_slider__list" ref="list">
       <span style="color: white;">{{ itemActive }}</span>
       <span style="color: white;">{{ numItemReal }}</span>
      <div
        class="v_slider__track"
        ref="track"
        :style="{width: width.track + 'px', transform: 'translate(-' + transform + 'px)', transition: 'transform ' + settings.timing + ' ' + settings.speed + 'ms'}"
      >
        <slot></slot>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    // Перемещение слайдера на величину свайпа
    // translateTrack(){
    //   if(!this.mouseDown){
    //     return;
    //   }
    //   // let oldSpeed = this.settings.speed;
    //   // this.settings.speed = 0;
    //   // console.log(this.dragDistance);
    //   this.oldTransform = this.transform;
    //   this.transform += this.dragDistance;
